defmodule Vocagraphy.AuthTest do
  use Vocagraphy.ConnCase
  alias Vocagraphy.Auth

  setup %{conn: conn} do
    conn =
      conn
      |> bypass_through(Vocagraphy.Router, :browser)
      |> get("/")
  ...
  test "authenticate_user continues when the current_user exists", %{conn: conn} do
    conn =
      conn
      |> assign(:current_user, %Vocagraphy.User{})
  ...
  test "login puts the user in the session", %{conn: conn} do
    login_conn =
      conn
      |> Auth.login(%Vocagraphy.User{id: 123})
      |> send_resp(:ok, "")
  ...
  test "logout drops out of the session", %{conn: conn} do
    logout_conn =
      conn
